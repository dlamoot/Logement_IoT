...
def accueil():
    ...
    fonctionnalites = [
        {"titre": "Consommation", "description": "Visualisez votre consommation d'électricité, d'eau, et plus encore.", "lien": "consommation.html"},
        {"titre": "État des Capteurs", "description": "Surveillez en temps réel vos capteurs et actionneurs connectés.", "lien": "etat_capteurs.html"},
        {"titre": "Économies Réalisées", "description": "Comparez vos dépenses et suivez vos économies sur le long terme.", "lien": "economies.html"},
        {"titre": "Configuration", "description": "Ajoutez ou configurez vos capteurs et actionneurs pour une meilleure efficacité.", "lien": "configuration.html"}
    ]
    return render_template('index.html', fonctionnalites=fonctionnalites)

@app.route('/consommation')
def consommation():
    """
    Récupère les données de consommation depuis la base de données.
    """
    conn = get_db_connection()
    ...
    # Transformer les résultats en un format utilisable par Jinja2
    consommation_data = [{"type": facture["type"], "montant": facture["total_montant"]} for facture in factures]

    # Rendre le template HTML et passer les données
    return render_template('consommation.html', consommation_data=consommation_data)

@app.route('/etat_capteurs')
def etat_capteurs():
    """
    Affiche l'état des capteurs et actionneurs avec leurs valeurs et mises à jour.
    """
    conn = get_db_connection()
    # Récupérer l'état des capteurs/actionneurs et leurs dernières valeurs
    ...
    # Rendre la page HTML avec les données des capteurs
    return render_template('etat_capteurs.html', capteurs_data=capteurs_data)

@app.route('/economies')
def economies():
    """
    Calcule et affiche les économies réalisées par rapport aux périodes précédentes.
    """
    conn = get_db_connection()
    # Simulation des données d'économies (à ajuster selon vos besoins)
    economies = [
        {"mois": "Janvier", "economie": 50},
        {"mois": "Février", "economie": 70},
        {"mois": "Mars", "economie": 100},
        {"mois": "Avril", "economie": 80},
        {"mois": "Mai", "economie": 120}
    ]
    conn.close()

    # Rendre la page HTML avec les données
    return render_template('economies.html', economies=economies)
    ...
    if request.method == 'POST':
        ...
        if action == "add_capteur":
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (id_piece, id_type, reference, port))
            conn.commit()

        elif action == "add_facture":
            # Ajouter une facture
            id_logement = request.form.get('logement')
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (id_logement, type_facture, date_facture, consommation))
            conn.commit()

        conn.close()
        return redirect('/configuration')

    # Récupérer les logements et les types de capteurs pour les formulaires
    logements = conn.execute("SELECT id_logement, adresse FROM Logement").fetchall()
    types_capteurs = conn.execute("SELECT id_type, nom_type FROM Type_Capteur_Actionneur").fetchall()
    conn.close()

    # Rendre le formulaire HTML
    return render_template('configuration.html', logements=logements, types_capteurs=types_capteurs)


    # Récupérer les types de capteurs/actionneurs pour le formulaire
    types_capteurs = conn.execute("""
        SELECT id_type, nom_type FROM Type_Capteur_Actionneur
    """).fetchall()
    conn.close()

    # Rendre la page HTML avec les types de capteurs
    return render_template('configuration.html', types_capteurs=types_capteurs)
